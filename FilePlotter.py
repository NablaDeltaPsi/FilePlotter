...
class MainGUI(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        try:
            self.parent.iconbitmap(GUINAME + '.ico')
        except:
            print("Did not find icon... continuing...")
    ...
    def init_config(self, update=False):
        if update:
            ...
            os.remove(GUINAME + '.conf')
        if not os.path.exists(GUINAME + '.conf'):
    ...
    def save_conf_file(self):
        if os.path.exists(GUINAME + '.conf'):  # in case it has been deleted during execution
