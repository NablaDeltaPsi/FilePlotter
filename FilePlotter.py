# required packages:
# os, time, tkinter, numpy, pandas, datetime
# matplotlib, tkinter, scipy, configparser, ctypes
...
def match_up_columns(xcols, ycols, yecols, limit):
    if len(xcols) > 50 or len(ycols) > 50 or len(yecols) > 50:
...
class MainGUI(tk.Frame):
    ...
    def load_data(self):
        ...
        for filenr in range(filenumber):
            ...
            print("input columns: ", xcols, ycols, yecols)

            # match up columns against each other (e.g. same xcol for different ycols)
            xcols, ycols, yecols = match_up_columns(xcols, ycols, yecols, col_limit)
            if self.check_integer_x.get():
                xcols = [-1] * len(ycols)
            print("use columns: ", xcols, ycols, yecols)
    ...
    def edit_data(self):
        ...
        # Columns should not be of type string from now on
        for f in range(len(self.data)):
            for c in range(len(self.data[f])):
